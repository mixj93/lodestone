...
// * 0 德鲁伊 436, 1 猎人 437, 2 法师 437, 3 圣骑士 440, 4 牧师 436, 5 盗贼 439, 6 萨满 434, 7 术士 436, 8 战士 436
const CLASS_NAMES = ['druid', 'hunter', 'mage', 'paladin', 'priest', 'rogue', 'shaman', 'warlock', 'warrior'];
// const CLASS_DISPLAY_NAMES = ['德鲁伊', '猎人', '法师', '圣骑士', '牧师', '盗贼', '萨满', '术士', '战士'];
...
async function yingdi(cn) {
  ...
  console.log(`${new Date()}  [yingdi] [${CLASS_NAMES[cn]}] Get ${allCards.length} items`);

  // * Generate JSON File
  const cardsJson = JSON.stringify(allCards);
  await fs.writeFile(`./data/yingdi-${CLASS_NAMES[cn]}.json`, cardsJson, 'utf8', function (err) {
    if (err) {
      console.log(`${new Date()}  [yingdi] [${CLASS_NAMES[cn]}] Error occured when saving JSON file: ${err}`);
    } else{
      console.log(`${new Date()}  [yingdi] [${CLASS_NAMES[cn]}] JSON file saved`);
    }
  });
  ...
  await fs.writeFile(`./data/yingdi-${CLASS_NAMES[cn]}.csv`, cardsCsvArr.join('\n'), 'utf8', function (err) {
    if (err) {
      console.log(`${new Date()}  [yingdi] [${CLASS_NAMES[cn]}] Error occured when saving CSV file: ${err}`);
    } else{
      console.log(`${new Date()}  [yingdi] [${CLASS_NAMES[cn]}] CSV file saved`);
    }
  });
